...
/**
 * floating point version of JSlider. replaces the original integer get/set
 * Minimum/Maximum/Value methods with the floating point versions: get/set
 * FloatMinimum/FloatMaximum/FloatValue. it is an error to call the original
 * versions or other methods related to them. note that unlike in the base
 * class, values set via setFloatValue(val) are allowed to be outside the
 * min/max range values. in those cases the slider thumb will clamp to the
 * appropriate slider end (just like in the base class) but the out-of-range
...
public class FloatSlider extends Scrollbar {
	 ...
	 * @param cur
	 *            - real valued initial value.
	 * @param vis
	 *            - same as in Scrollbar base class.
	 * @param min
	 *            - real valued range minimum.
	 * @param max
	 *            - real valued range maximum.
	 * @param resolution
	 *            - number of descrete slider positions.
	 * @param log
	 *            - log scale if true, linear otherwise.
	 */
	public FloatSlider(int orientation, double cur, int vis, double min, double max, int res, boolean log) {
		...
		addAdjustmentListener(new AdjustmentListener() {
			public void adjustmentValueChanged(AdjustmentEvent ae) {
				int ival = FloatSlider.super.getValue(), vis = FloatSlider.super.getVisibleAmount(),
						min = FloatSlider.super.getMinimum(), max = FloatSlider.super.getMaximum();
				double dval = transformRange(false, min, max - vis, ival, isLogScale, minFloat, maxFloat);
	...
	/**
	 * returns the closest integer in the range of the actual int extents of the
	 * base Scrollbar.
	 */
	...
	public void setFloatValue(double newcur) {
		...
		curFloat = newcur;
		// update the view
		super.setValues(rangeValue(newcur), super.getVisibleAmount(), super.getMinimum(), super.getMaximum());
	}
