...
public class JSliderDouble extends JSlider {
	 ...
	 */
	public JSliderDouble(int orientation, double cur, double min, double max, int resolution, boolean log) {
		super(orientation, 0, resolution, 0);
		isLogScale = log;
		setAll(min, max, cur);

		double length = max - min;
		double delta = length / resolution;
		...
		for(int i=0; i< resolution - 1; i++) {
			...
			Double tick = length + ((i + 1) * delta);
			Integer tickCode = (int) (tick * resolution);
			String label = df.format(tick);

			System.out.println(i + " i " + " tick " + tick + " tickCode: " + tickCode);
			
			labelTable.put( new Integer( tickCode ), new JLabel(label) );
